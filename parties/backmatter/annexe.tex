...
Actuellement, lorsque l’utilisateur crée un élément du \gls{wemi} depuis une table (films, manifestation, œuvre), la création supplémentaire d’un lien avec les autres niveaux du \gls{wemi} n’est pas rendue obligatoire par l’application.  
...
Si la collection est particulière, par exemple pour une institution qui ne décrit pas la manifestation, il faut permettre la possibilité d’enlever cette obligation de lien entre les trois niveaux et laisser seulement l’encart existant. 
...
\subsubsection{Gestions des erreurs}

Deux logiques : la possibilité pour le processus d’indiquer qu’il ne comprend pas la requête utilisateur afin d’éviter la création d’une requête erronée. La possibilité pour l’utilisateur de signaler un problème. 

En cas d’incompréhension de la requête utilisateur et d’incertitude sur la requête à générer. La fonctionnalité doit :  

Ne pas lancer la requête car il n’est pas certain.  

Identifier le point bloquant  

Signaler l’erreur par un code couleur : 

Le mot posant un problème est surligné en rouge lorsque incompréhension totale. 

Le mot pose un problème est surligné en orange lorsqu’il y a une incertitude sur le sens.  

L’utilisateur doit pouvoir écrire dans la barre de recherche un complément à la requête d’origine. 

Le complément s’envoie au processus comme une requête classique. C’est l’interprétation qui change.  

S'il y a toujours une erreur, on revient à l’étape précédente jusqu’à résolution de l’erreur.  

Si l’utilisateur tombe dans une boucle infinie et décide d’arrêter le traitement, l’absence d’activité dans la barre de recherche dans les 30 secondes met fin au processus.  

Si le complément amène à résoudre l’erreur, le processus suit son cours. 
...
4 champs sont affichés :  

Champ Titre du rapport : l’utilisateur saisi un titre pour le rapport d’erreur. Permet de le rendre traçable. 

Champ requêtes : rempli automatiquement, il contient la requête utilisateur (la phrase de départ) + le résumé de la recherche générée par IA. Non visible par l’utilisateur, y figure aussi la requête ElasticSearch générée.  

Champ date : rempli automatiquement avec le jour de requêtage 

Champ description : l’utilisateur explique pourquoi cela ne lui convient pas et ce qu’il attendait.  
...
Envoie le rapport d’incident au LLM, l’historise et l’intègre dans une base de données pour entraînement. 

Génère une notification verte à l’endroit habituel : “Rapport d’erreur envoyé et enregistré”. 

“Annuler” 

Annule l’opération en cours. 

Génère une notification orange à l’endroit habituel : “Rapport d’erreur annulé”. 



Note :  

À terme, un système de suivi des rapports pourra être implémenté au sein du widget d’historique des rapports d’erreur. Chaque rapport d’erreur pourra être associé à un statut (ex : En attente, Pris en compte, Outil amélioré) afin que l’utilisateur puisse suivre la résolution de l'erreur et l'évolution de l'affinage de l’outil depuis son tableau de bord. Cette fonctionnalité n’est pas incluse en tant que telle dans la présente spécification en raison d’un manque de visibilité technique sur les modalités de réentraînement du modèle. 

\subsubsection{Fonctionnalité détaillée}

Requête mal formulée ou incertaine : 
...
Résultats de recherche invalides :
