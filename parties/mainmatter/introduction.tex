...
Dès lors, dans quelle mesure la variabilité du modèle de données \gls{FRBR}, adapté aux spécificités des archives audiovisuelles, conditionne-t-elle la mise en place d'une segmentation et d'une indexation automatiques pour ce type d'archives ? Les outils existants permettent-ils l'automatisation de tâches documentaires dédiées à de telles collections ? Quel est la direction adoptée par un éditeur de gestion des collections afin de concilier pratiques documentaires appliquées à des corpus spécifiques, et une éthique d'usage au sein d'un même processus ?
